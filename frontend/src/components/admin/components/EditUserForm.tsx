...
import { updateUserById, type User, updateUserByIdWithImage, getUsers } from '../../../services/users'
...
const userFormResources = {
    fr: {
        userForm: {
            ...
            buttons: { suggest: "Suggérer", cancel: "Annuler", saving: "Enregistrement...", save: "Enregistrer", removePhoto: "Supprimer la photo" },
            errors: {
    ...
    en: {
        userForm: {
            ...
            buttons: { suggest: "Suggest", cancel: "Cancel", saving: "Saving...", save: "Save", removePhoto: "Remove photo" },
            errors: {
    ...
    ar: {
        userForm: {
            ...
            buttons: { suggest: "اقتراح", cancel: "إلغاء", saving: "جاري الحفظ...", save: "حفظ", removePhoto: "حذف الصورة" },
            errors: {
...
export default function EditUserForm({
    ...
    useEffect(() => {
        ...
        async function loadUsers() {
            try {
                // Fetch potential parents separately
    ...
    }, [initial?.id_father, initial?.id_mother])
    ...
    useEffect(() => {
        setForm(f => ({ ...f, image_url: initial?.image_url ?? '' }))
    ...
    async function submitForm() {
        ...
        try {
            ...
            const payload: {
                ...
                contribution_tier?: 'LEVEL1' | 'LEVEL2' | 'LEVEL3' | 'LEVEL4' | null
                id_father?: number
                id_mother?: number
                isactive?: number
                isfirstlogin?: number
                ...
                gender: ((form as any).gender === 'male' || (form as any).gender === 'female') ? (form as any).gender : null,
            ...
            if (imageFile) {
                payload.image_url = form.image_url ? String(form.image_url) : undefined
            } else if (removeImage) {
            ...
            if (imageFile) {
                updated = await updateUserByIdWithImage(id, payload, imageFile)
            } else {
                updated = await updateUserById(id, payload)
            }
    ...
    const filterCandidates = (list: User[]) => list.filter(u => {
        if (initial?.id && u.id === initial.id) return false
    ...
    const filteredFathers = filterCandidates(potentialFathers)
    const filteredMothers = filterCandidates(potentialMothers)
    ...
    if (initial?.id_father) {
        // Use potentialFathers
        const fu = potentialFathers.find(u => u.id === initial.id_father)
        if (fu && !fatherOptions.some(o => o.value === fu.id)) {
            fatherOptions = [...fatherOptions, toOption(fu as User)]
        }
    }

    let motherOptions = [...filteredMothers.map(toOption)]
    if (initial?.id_mother) {
        // Use potentialMothers
        const mu = potentialMothers.find(u => u.id === initial.id_mother)
        if (mu && !motherOptions.some(o => o.value === mu.id)) {
            motherOptions = [...motherOptions, toOption(mu as User)]
        }
    }

    const previewUrl = removeImage ? null : (localPreview ?? (form.image_url ? form.image_url : null))
        ...
        <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div className="card-header bg-light bg-gradient border-bottom py-3 px-4">
                <div className="d-flex align-items-center">
                    ...
                    <div>
                        <h5 className="card-title fw-bold text-dark mb-0">{t('userForm.title.edit')}</h5>
                        <p className="text-muted small mb-0">{t('userForm.subtitle.edit')}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="row g-4">
                    <div className="col-lg-4">
                        <div className="text-center mb-4">
                            ...
                            {previewUrl && (
                                <button type="button" className="btn btn-sm btn-outline-danger mt-2" onClick={handleRemoveImage}>
                                    <i className="bi bi-trash me-1"></i>{t('userForm.buttons.removePhoto')}
                                </button>
                            )}
                        </div>
                                ...
                                <div className="col-md-6">
                                    ...
                                    <div className="d-flex gap-3 align-items-center">
                                        <div className="form-check">
                                            <input className="form-check-input" type="radio" name="gender" id="gender-male" checked={(form as any).gender === 'male'} onChange={() => setForm(f => ({ ...f, gender: 'male' as any }))} />
                                            <label className="form-check-label" htmlFor="gender-male">{i18n.language === 'fr' ? 'Homme' : i18n.language === 'ar' ? 'ذكر' : 'Male'}</label>
                                        </div>
                                        <div className="form-check">
                                            <input className="form-check-input" type="radio" name="gender" id="gender-female" checked={(form as any).gender === 'female'} onChange={() => setForm(f => ({ ...f, gender: 'female' as any }))} />
                            ...
                            <div className="row g-3">
                                ...
                                <div className="col-md-6">
                                    ...
                                    <div className={`input-group ${phoneError ? 'is-invalid' : ''}`}>
                                        <PhoneInput international defaultCountry="GN" value={phone} onChange={(val) => { setPhone(val); setPhoneError(null) }} className="form-control d-flex" inputClassName="form-control border-0 shadow-none bg-transparent" />
                                    </div>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="col-12">
                                ...
                                <div className="input-group">
                                    ...
                                    <input className="form-control border-start-0 ps-0" placeholder={suggestedUsername || t('userForm.placeholders.username')} value={form.username || ''} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} style={{ transition: 'background-color 300ms', backgroundColor: isTypingUsername ? '#eaf4ff' : undefined }} />
                            ...
                            <div className="row g-3">
                                <div className="col-md-6">
                                    ...
                                    <Select
                                        ...
                                        value={fatherId !== null ? (fatherOptions.find(o => o.value === fatherId) ?? null) : null}
                                        onChange={(opt) => setFatherId(opt ? (opt as any).value : null)}
                                ...
                                <div className="col-md-6">
                                    ...
                                    <Select
                                        ...
                                        value={motherId !== null ? (motherOptions.find(o => o.value === motherId) ?? null) : null}
                                        onChange={(opt) => setMotherId(opt ? (opt as any).value : null)}
                                        styles={{ control: (base) => ({ ...base, borderColor: '#dee2e6', boxShadow: 'none' }) }}
                                    />
                                </div>
                            </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="form-check pointer-events-none">
                                                <input className="form-check-input" type="checkbox" id={`role-${role.id}`} checked={isSelected} readOnly />
