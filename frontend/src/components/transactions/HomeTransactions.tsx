...
export default function HomeTransactions() {
    ...
    return (
        <div className="container py-3">
                ...
                <table className="table table-hover align-middle border-light shadow-sm bg-white rounded-3 overflow-hidden" style={{borderCollapse: 'separate', borderSpacing: '0'}}>
                    ...
                    <tbody className="border-top-0">
                        {loading ? (
                            ...
                            const isTreasury = currentUser?.roles?.some(r => r.role === 'treasury');
                            ...
                            // Modification Logic: Only SAVED
                            const canEdit = tx.status === 'SAVED';

                            // Deletion Logic:
                            // Basic rule: Only SAVED
                            // Treasury rule: Also PENDING (but not VALIDATED or others)
                            let canDelete = false;
                            if (tx.status === 'SAVED') {
                                canDelete = true;
                            } else if (tx.status === 'PENDING' && isTreasury) {
                                canDelete = true;
                            }
                                ...
                                <tr key={tx.id} className="">
                                    ...
                                    <td className="text-end pe-4">
                                        ...
                                        <button
                                            ...
                                            disabled={!canEdit}
                                            title={canEdit ? t('transactions.home.edit') : t('transactions.home.onlyPending')}
                                        >
            ...
            <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title={t('transactions.home.addTitle')} size="lg">
                <AddTransaction
                    onSuccess={() => {
                        setShowAdd(false)
                        // refresh list
            ...
            <Modal isOpen={showEditId !== null} onClose={() => setShowEditId(null)} title={t('transactions.home.updateTitle')} size="lg">
                {showEditId !== null && (
                    <UpdateTransaction
                        id={showEditId}
                        onSuccess={() => {
